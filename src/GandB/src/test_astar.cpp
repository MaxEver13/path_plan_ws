...
int main(int argc, char** argv)
{
    ...
    while (ros::ok())
    {
        ...
        const int th = 1;
        for (float z = 0.0; z < 3.5; z += resolution)
        ...
        vector<Eigen::Vector3d> kino_path = path_finder->getKinoTraj(resolution);

        // cout << "path size: " << path.size() << endl;
        // cout << "path_nodes size: " << path_nodes.size() << endl;
        cout << "kino_path size: " << kino_path.size() << endl;
        displayPathWithColor(kino_path, resolution, 1, 1);
        // displayVisitedNodes(visited_nodes, resolution);
        // displayVisitedNodes(path_nodes, resolution);

        // for (int i = 0; i < kino_path.size(); i++) {
        //     cout << kino_path[i].transpose() << endl;
        // }
        ...
        if (samples.cols() == 0) {
            ROS_ERROR("[getSamples] is null sample !!!");
            return -1;
        }
        ...
        // cout << "sample head:" << samples.col(0).transpose() << endl;
        // cout << "sample tail:" << samples.col(samples.cols()-1).transpose() << endl;
        ...
        cout << "samples size: " << v_sample.size() << endl;
        ...
        extend_samples.resize(3, K + 6); // k=samples.size() - 2
        extend_samples.block(0, 0, 3, K + 2) = samples;
        extend_samples.col(K + 2) = sv; // 起点速度
        extend_samples.col(K + 3) = ev; // 终点速度
        extend_samples.col(K + 4) = Eigen::Vector3d::Zero(); // 起点加速度
        extend_samples.col(K + 5) = Eigen::Vector3d::Zero(); // 终点加速度
        getControlPointEqu5(extend_samples, ts, control_pts);
        // getControlPointEqu3(extend_samples, ts, control_pts); // 有问题

        cout << "control_pts.cols(): " << control_pts.cols() << ", control_pts.rows():" << control_pts.rows() << endl;

        // for (int i = 0; i < int(control_pts.rows()); ++i)
        // {    
        //     cout << control_pts.block(i, 0, 1, 3) << endl; 
        // }
        ...
        while (true)
        {
            ...
            // cout << "============ vc ============ " << endl;
            // cout << "vc.cols(): " << vc.cols() << ", vc.rows():" << vc.rows() << endl;

            // for (int i = 0; i < int(vc.rows()); ++i)
            // {    
            //     cout << vc.block(i, 0, 1, 3) << endl; 
            // }            

            // cout << "============ ac ============ " << endl;
            // cout << "ac.cols(): " << ac.cols() << ", ac.rows():" << ac.rows() << endl;

            // for (int i = 0; i < int(ac.rows()); ++i)
            // {    
            //     cout << ac.block(i, 0, 1, 3) << endl; 
            // } 
        ...
        t2 = ros::Time::now();
        ROS_WARN("time in bspline: %f", (t2 - t1).toSec());
        // cout << "time in bspline:" << (t2 - t1).toSec() << endl;
